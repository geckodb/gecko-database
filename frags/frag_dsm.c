...
static inline void this_fragment_dsm_dipose(frag_t *self);
static inline tuplet_t *this_fragment_dsm_open(frag_t *self);
...
static inline tuplet_t *fragment_dsm_open_internal(frag_t *self, size_t slot_id);
...
static inline void tuplet_rebase(tuplet_t *tuplet, frag_t *frag, size_t slot_id)
{
    ...
    tuplet->attr_base = frag->tuplet_data + (slot_id * gs_field_type_sizeof(gs_schema_attr_by_id(frag->schema, 0)->type));
}
...
static inline tuplet_t *fragment_dsm_open_internal(frag_t *self, size_t slot_id)
{
    ...
    if (self->ntuplets > 0) {
        ...
        *result = (tuplet_t) {
        ...
        tuplet_rebase(result, self, slot_id);
    }
    return result;
}
...
{
    return fragment_dsm_open_internal(self, 0);
}
...
static inline tuplet_t *this_fragment_insert(struct frag_t *self, size_t ntuplets)
{
    ...
    return fragment_dsm_open_internal(self, return_tuplet_id);
}
