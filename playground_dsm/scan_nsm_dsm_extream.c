...
int main() {
    FILE *result_file = fopen("results_nsm_dsm5.csv", "a+");
    ...
    size_t num_fields = 1;
    size_t fields_conf[] = {1, 10000};
    size_t num_tuplets = 10000000;

    for (int f = 1; f < 2; ++f) {

        for (size_t n_tuplets = num_tuplets; n_tuplets <= num_tuplets; n_tuplets+= (num_tuplets -1 )) {
            ...
            for (size_t field_conf_idx = 0; field_conf_idx < 1; field_conf_idx++) {
