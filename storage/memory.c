...
{
    require_non_null(buf);
    WARN_IF((id >= buf->page_anticache.next_page_id), "Requested page id '%d' might not from this pool", id);
...
//    panic_if((buf == NULL), BADARGNULL, to_string(buf));
//    panic_if((buf->page_anticache.hot_store == NULL), UNEXPECTED, "page register in buf");
//    panic_if (!(dict_contains_key(buf->page_anticache.hot_store, &id)), BADPAGEID, id);
//
...
//
//    return dict_remove(buf->page_anticache.hot_store, 1, &id);
//}
