                                ...
                                const gs_tuple_id_t *tuple_ids, size_t ntuple_ids)
{
    ...
    *dummy = true;
    ...
    for (const gs_grid_t *grid = gs_grid_cursor_next(smaller); grid != NULL; grid = gs_grid_cursor_next(NULL)) {
        gs_grid_t *grid_cp = apr_pmemdup(pool, grid, sizeof(gs_grid_t));
        apr_hash_set(hash_table, grid_cp, sizeof(gs_grid_t), dummy);
    }
...
const gs_attr_id_t *gs_table_attr_id_to_frag_attr_id(const gs_grid_t *grid, gs_attr_id_t table_attr_id)
{
    ...
    const gs_attr_id_t *frag_attr_id = apr_hash_get(grid->schema_map_indicies, &table_attr_id, sizeof(gs_attr_id_t));
    return frag_attr_id;
}
...
void gs_grid_print(FILE *file, const gs_table_t *table, gs_grid_id_t grid_id, size_t row_offset, size_t limit)
{
    ...
    GS_REQUIRE_NONNULL(table)
    const gs_grid_t *grid = gs_grid_by_id(table, grid_id);
...
void gs_table_grid_list_print(FILE *file, const gs_table_t *table, size_t row_offset, size_t limit)
{
    ...
    gs_frag_t *frag = frag_new(print_schema, num_tuples, FIT_HOST_NSM_VM);
...
void gs_table_print(FILE *file, const gs_table_t *table, size_t row_offset, size_t limit)
{
    ...
    frag_print(file, gs_grid_by_id(molten_table, 0)->frag, row_offset, limit);
...
void gs_table_structure_print(FILE *file, const gs_table_t *table, size_t row_offset, size_t limit)
{
    ...
    do {
        ...
        while (num_attr--) {
            ...
            for (gs_grid_id_t grid_id = 0; grid_id < gs_table_num_of_grids(table); grid_id++) {
                const gs_grid_t *grid = gs_grid_by_id(table, grid_id);
                for (gs_attr_id_t grid_attr_id = 0; grid_attr_id < gs_grid_num_of_attributes(grid); grid_attr_id++) {
                    const char *g_attr_name = gs_schema_attr_by_id(frag_schema(grid->frag), grid_attr_id)->name;
                                  ...
                                  const gs_tuple_id_interval_t *tuple_ids, size_t ntuple_ids, enum frag_impl_type_t type)
{
    ...
    gs_schema_t *grid_schema = gs_schema_subset(table->schema, attr, nattr);
    assert (grid_schema);
